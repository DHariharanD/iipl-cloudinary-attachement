sap.ui.define(
    ...
    function (Fragment, JSONModel, MessageToast, Filter, FilterOperator) {
        ...
        that = {
            ...
            cbsResAdding: async function (oContext) {
                _ctx = oContext;
            ...
            onVHConfirm: async function () {
                if (!_ctx) {
                    return sap.m.MessageToast.show("Error: No Context Found");
                }
                ...
                console.log("CTX PATH:", oCtx.getPath());
                console.log("CTX ID:", oCtx.getProperty("ID"));
                ...
                if (aItemsToAdd.length === 0) {
                    oDlg.setBusy(false);
                    return sap.m.MessageToast.show("Please select at least one new item");
                }
                ...
                aItemsToAdd.forEach(oData => {
                    oChildList.create({
                        ...
                        cbsNode_ID: oCtx.getProperty("ID"),
                ...
                try {
                    ...
                    sap.m.MessageToast.show("Resource added successfully");
                    oDlg.close();
